(function($)   {  // 点击选择所有按钮后执行的操作 
			...
			$.ajax({      
				...
				            success: function(data) {       
					...
					if (isNullOrEmpty(str)) {        
						...
						$("#" + oSelect.allSelectCheckbox).attr("checked", true);        
						// 将表格所有checkbox改为勾选状态            
						$("#" + oSelect.tableName).find("tbody").find("tr").each(            function(index, element) {             
							$(element).find("td:first").find("input").attr("checked", "true");           
						});       
					}        
				}     
			});  
	...
	$.fn.allSelectCheckboxClick = function(oSelect)  {  
		...
		if ($("#" + oSelect.allSelectCheckbox).attr("checked"))   {   
			$("[name=" + oSelect.selectCheckbox + "]:checkbox").attr("checked", true);      
			...
			$("[name=" + oSelect.selectCheckbox + "]:checkbox").removeAttr("checked");   
			for (var i = 0; i < chks.length; i++)    {    
				for (var x in chks[i].attributes)     {     
	...
	$.fn.selectCheckboxClick = function(element, oSelect)  {  
		...
		for (var i = 0; i < chks.length; i++)   {   
			if (chks[i].checked == false)    {    
				$("#" + oSelect.allSelectCheckbox).attr("checked", false);    
				break;   
			}   
			selectedChks++;     
		}    
